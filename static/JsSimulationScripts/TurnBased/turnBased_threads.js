  ...
  var collaborator_0_id = 0;
  var collaborator_0_next_instruction_serial = 0;
  var collaborator_0_access_requested_already = false;
  var collaborator_0_is_in_waiting_state = false;

  function collaborator_0() {
    //still some instructions to execute
    if (collaborator_0_next_instruction_serial <= INSTRUCTIONS_PER_COLLABORATOR) {
      //it is my floor
      if (currentFloorOwner == collaborator_0_id) {
        if (collaborator_0_next_instruction_serial % 2 == 0) { //thinking time
          var thinkingTime = workflow_instructions[collaborator_0_id][collaborator_0_next_instruction_serial];
          //if thinking time is too much, release floor for others
          if (thinkingTime >= 5000) {
            collaborator_0_next_instruction_serial++;
            console.log("FLOOR_RELEASED -> " + collaborator_0_id);
            collaborator_0_access_requested_already = false;
            releaseFloor(collaborator_0_id);
                      setTimeout(function() {
            collaborator_0();
          }, 1000);
            ...
            collaborator_0_next_instruction_serial++;
            console.log("THINKING -> " + collaborator_0_id);
            setTimeout(function() {
              collaborator_0();
            }, thinkingTime);
          }
        } else {
          console.log("UPDATE -> " + collaborator_0_id);
          collaborator_0_next_instruction_serial++; //lets try to move for next instruction
          setTimeout(function() {
            collaborator_0();
          }, 100);
        }
        ...
        if (collaborator_0_access_requested_already == false) {
        	collaborator_0_access_requested_already = true;
          newFloorRequest(collaborator_0_id);
          console.log("FLOOR_REQUESTED -> " + collaborator_0_id);
          setTimeout(function() {
            collaborator_0();
          }, 1000);
        } else { //floor requested already and I am just waiting now
          console.log("WAITING -> " + collaborator_0_id + " CF->"+currentFloorOwner+"<-" + "RL0" + floorRequestQueue[0]);
      ...
      console.log("END -> " + collaborator_0_id+ " CF->"+currentFloorOwner+"<-" + "RL0" + floorRequestQueue[0]);
      releaseFloor(collaborator_0_id);
    }
  ...
  var collaborator_1_id = 1;
  var collaborator_1_next_instruction_serial = 0;
  var collaborator_1_access_requested_already = false;
  var collaborator_1_is_in_waiting_state = false;

  function collaborator_1() {
    //still some instructions to execute
    if (collaborator_1_next_instruction_serial <= INSTRUCTIONS_PER_COLLABORATOR) {
      //it is my floor
      if (currentFloorOwner == collaborator_1_id) {
        if (collaborator_1_next_instruction_serial % 2 == 0) { //thinking time
          var thinkingTime = workflow_instructions[collaborator_1_id][collaborator_1_next_instruction_serial];
          //if thinking time is too much, release floor for others
          if (thinkingTime >= 5000) {
            collaborator_1_next_instruction_serial++;
            console.log("FLOOR_RELEASED -> " + collaborator_1_id);
            collaborator_1_access_requested_already = false;
            releaseFloor(collaborator_1_id);
                      setTimeout(function() {
            collaborator_1();
          }, 1000);
            ...
            collaborator_1_next_instruction_serial++;
            console.log("THINKING -> " + collaborator_1_id);
            setTimeout(function() {
              collaborator_1();
            }, thinkingTime);
          }
        } else {
          console.log("UPDATE -> " + collaborator_1_id);
          collaborator_1_next_instruction_serial++; //lets try to move for next instruction
          setTimeout(function() {
            collaborator_1();
          }, 100);
        }
        ...
        if (collaborator_1_access_requested_already == false) {
        	collaborator_1_access_requested_already = true;
          newFloorRequest(collaborator_1_id);
          console.log("FLOOR_REQUESTED -> " + collaborator_1_id);
          setTimeout(function() {
            collaborator_1();
          }, 1000);
        } else { //floor requested already and I am just waiting now
          console.log("WAITING -> " + collaborator_1_id+ " CF->"+currentFloorOwner+"<-" + "RL0" + floorRequestQueue[0]);
          //keep checking if I have got the floor
          setTimeout(function() {
            collaborator_1();
          }, 1000);
      ...
      console.log("END -> " + collaborator_1_id+ " CF->"+currentFloorOwner+"<-" + "RL0" + floorRequestQueue[0]);
      releaseFloor(collaborator_1_id);
    }
